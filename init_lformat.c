...
static	void	init_owngp(t_dir *list, t_max *max)
{
	...
	while (cp)
	{
		(cp->long_f).owner = ft_strdup((getpwuid(cp->buf.st_uid))->pw_name);
		cp = cp->next;
	}
	cp = list;
	while (cp)
	{
		(cp->long_f).group = ft_strdup((getgrgid(cp->buf.st_gid))->gr_name);
		cp = cp->next;
	}
...
void			init_lformat(t_dir *list, t_max *max, char *flags)
{
	...
	while (cp)
	{
		if (ft_strchr(flags, 'u'))
			cp->long_f.change_tm = ft_strdup(ctime((const time_t *)\
